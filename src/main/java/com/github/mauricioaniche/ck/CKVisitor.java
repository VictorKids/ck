...
public class CKVisitor extends ASTVisitor {
	...
	public boolean visit(TypeDeclaration node) {
		...
		if(!classes.isEmpty()) {			
			...
			if (!classes.peek().methods.isEmpty())
				classes.peek().methods.peek().methodLevelMetrics.stream().map(CKASTVisitor.class::cast).forEach(ast -> ast.visit(node));
	...
	public void endVisit(TypeDeclaration node) {
		...
		classes.peek().classLevelMetrics.stream().map(CKASTVisitor.class::cast).forEach(ast -> ast.endVisit(node));
	...
	public void endVisit(MethodDeclaration node) {
		...
		classes.peek().classLevelMetrics.stream().map(CKASTVisitor.class::cast).forEach(ast -> ast.endVisit(node));
		classes.peek().methods.peek().methodLevelMetrics.stream().map(CKASTVisitor.class::cast).forEach(ast -> ast.endVisit(node));
